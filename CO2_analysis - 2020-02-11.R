...
plot(NULL, xlim=c(1959,2040), ylim=c(0,30),
     xlab='', ylab=expression('Atmospheric growth rate (GtCO'[2]*'/year)'))
...
par(cex = cex); par(mar = c(2.2, 4.1, 1.1, 1.1))
plot(NULL, xlim=future$t[c(1,N)], ylim=c(400,500), xlab='', ylab=expression('Atmospheric CO'[2]*' (ppm)'))
...
png(filename = filename, width = wd, height = ht, res = 300)
par(cex = cex); par(mar = c(2.2, 4.1, 1.1, 1.1))
plot(NULL, xlim=future$t[c(1,N)], ylim=c(0, 35), xlab='', ylab=expression('Atmospheric growth rate (Gt CO'[2]*')'))
